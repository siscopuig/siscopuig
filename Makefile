...
init:
	@$(shell cp -n $(shell pwd)/siscopuig-web/app/composer.json.dist $(shell pwd)/siscopuig-web/app/composer.json 2> /dev/null)
...
clean:
	...
	@rm -Rf siscopuig-web/app/vendor
	@rm -Rf siscopuig-web/app/composer.lock
	@rm -Rf siscopuig-web/app/doc
	@rm -Rf siscopuig-web/app/report
	@rm -Rf etc/ssl/*
...
composer-up:
	@docker run --rm -v $(shell pwd)/siscopuig-web/app:/app composer update
